...
function filter(json_data, key) {
    ...
    var moreandfirstlangwords = new Array();
    var moreandsecondlangwords = new Array();


    for (var i = 0; i < firstlangwords.length; i++) {
        if (secondlangwords[i] == key) {
            matchedfirstlangwords.push(firstlangwords[i]);
            matchedsecondlangwords.push(secondlangwords[i]);
        }
        if (secondlangwords[i].startsWith(key) && secondlangwords[i] != key) {
            morefirstlangwords.push(firstlangwords[i]);
            moresecondlangwords.push(secondlangwords[i]);
        }

        if (secondlangwords[i].includes(key) == true && secondlangwords[i].startsWith(key) == false && secondlangwords[i] != key) {
            moreandfirstlangwords.push(firstlangwords[i]);
            moreandsecondlangwords.push(secondlangwords[i]);
        }

    }

    var filteredfirstlangwords = matchedfirstlangwords.concat(morefirstlangwords).concat(moreandfirstlangwords);
    var filteredsecondlangwords = matchedsecondlangwords.concat(moresecondlangwords).concat(moreandsecondlangwords);

    if (filteredfirstlangwords.length > 0) {
        if (filteredfirstlangwords.length > 100) {
            for (var j = 0; j < 100; j++) {
                app.innerHTML += '<div class="word focusable" tabindex="' + (j + 1) + '" speakword="' + filteredfirstlangwords[j] + '">' + filteredsecondlangwords[j] + ' = ' + filteredfirstlangwords[j] + '</div>';
