...
function filter(json_data, key) {
    ...
    for (var i = 0; i < firstlangwords.length; i++) {
        ...
        if (secondlangwords[i].startsWith(key) == true && secondlangwords[i] != key) {
            morefirstlangwords.push(firstlangwords[i]);
            moresecondlangwords.push(secondlangwords[i]);
        }
        if (secondlangwords[i].startsWith(key) != true && secondlangwords[i] != key&& secondlangwords[i].includes(key)) {
            morefirstlangwords.push(firstlangwords[i]);
            moresecondlangwords.push(secondlangwords[i]);
        }
    }
...
document.body.addEventListener("keyup", () => { getKaiAd({
    ...
    app: 'English-Bangla-Dictionary',
    slot: 'English-Bangla-Dictionary',
