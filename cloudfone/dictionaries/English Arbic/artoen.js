...
function filter(json_data, key) {
    ...
    for (var i = 0; i < secondlangwords.length; i++) {
    ...
    if (filteredfirstlangwords.length > 0) {
        if (filteredfirstlangwords.length > 100) {
            for (var j = 0; j < 100; j++) {
                app.innerHTML += '<div class="word focusable" tabindex="' + (j + 1) + '" speakword="' + filteredfirstlangwords[j] + '"><font style="text-direction: rtl">' + filteredsecondlangwords[j] + '</font> = ' + filteredfirstlangwords[j] + '</div>';
...
document.body.addEventListener("keyup", () => {
    getKaiAd({
        ...
        app: 'English-Arabic-Dictionary',
        slot: 'English-Arabic-Dictionary',
